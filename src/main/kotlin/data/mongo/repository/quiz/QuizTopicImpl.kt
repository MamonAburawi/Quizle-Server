...
import com.data.entity.quiz.QuizQuestionEntity
import com.data.entity.quiz.QuizTopicEntity
...
class QuizTopicImpl(
    ...
    override suspend fun getQuizTopicById(id: String?): Result<QuizTopic, AppException> {
        if (id.isNullOrBlank()) {
    ...
    override suspend fun getAllQuizTopic(): Result<List<QuizTopic>, AppException> {
        return try {
            val sort = Sorts.ascending(QuizTopicEntity::titleEnglish.name)
            val topics = quizTopicCollection
                .find()
    ...
    override suspend fun deleteQuizTopicById(id: String?): Result<Unit, AppException> {
        if (id.isNullOrBlank()) {
            Result.Failure(DataException.InvalidDataProvidedException())
        }
        return try {
    ...
    override suspend fun upsertQuizTopic(quizTopic: QuizTopic): Result<Unit, AppException> {
        val id = quizTopic.id
        return try {
