...
fun RequestValidationConfig.validateQuizQuestion() {
    validate<QuizQuestion> { quizQuestion ->
        when {
            ...
            // Rule 3: There must be at least 3 incorrect answers
            quizQuestion.inCorrectAnswers.size < 3 -> // Changed from <= 2 for clarity, as you need "at least 3"
                createInvalidResult(
                    messageEn = "There must be at least 3 incorrect answers.",
                    messageAr = "يجب أن يكون هناك 3 إجابات خاطئة على الأقل."
                )

            // Rule 4: Incorrect answers must not be empty
            ...
            // Rule 5: Topic code must be a positive value
